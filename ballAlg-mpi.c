...
#pragma omp task
    ...
    long new_node_id = id < center_proc ? left : right;
    ...
    long new_team_set = team_set / 2 + (team_set % 2 && id >= center_proc);
    ...
    build_tree(pts, new_team, nodes, sorted_set, new_team_set, new_node_id);
}
